...
import tech.wedev.pratice.design.combination.order.auditlog.OrderLogProcessor;
import tech.wedev.pratice.design.combination.order.auditlog.PayLogProcessor;
...
public class OrderServiceImpl implements OrderService {
    ...
    //不会对调用层产生任何代码的改动。
    // 调用层使用我们的 pay 模块，无需关心实现的逻辑，只需要将入参传给我们的pay模块即可。
    ...
    private boolean changeStateAction(Message<OrderStateChangeAction> message, Order order) {
        try {
            ...
            stateMachinePersister.restore(orderStateMachine, order); // 恢复
